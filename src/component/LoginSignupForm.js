...
const LoginSignupForm = ({ closeForm }) => {
  ...
  const evaluatePasswordRequirements = (password) => {
    ...
    if (!isLogin && password.length === 0) {
      ...
      !isLogin &&
      (!hasSpecialChar || !hasCapitalLetter || !hasNumber)
    ) {
      setPasswordAlert(
        "Password must include at least one special character, one uppercase letter, and one number."
      );
    } else {
      setPasswordAlert("");
    }
    ...
    if (!isLogin) {
      try {
        const response = await fetch(
          `http://localhost:5000/check-username/${newUsername}`
        );
          ...
          <form onSubmit={submitForm}>
            {!isLogin && (
              <div className="form-group">
                ...
                {usernameExists && (
                  <div className="error-message">{usernameExists}</div>
                )}
              </div>
            )}
            <div className="form-group">
            ...
            <div className="form-group">
              ...
              />
              {passwordAlert && (
                <div className="error-message">{passwordAlert}</div>
              )}
              {!isLogin && (
                <div className="password-requirements">
                  <p className={requirements.length ? "valid" : "invalid"}>
                    <i
                      className={`fa ${
                        requirements.length ? "fa-check" : "fa-times"
                      }`}
                    ></i>
                  ...
                  <p className={requirements.specialChar ? "valid" : "invalid"}>
                    <i
                      className={`fa ${
                        requirements.specialChar ? "fa-check" : "fa-times"
                      }`}
                    ></i>
                    At least one special character
                  </p>
                  <p
                    className={requirements.capitalLetter ? "valid" : "invalid"}
                  >
                    <i
                      className={`fa ${
                        requirements.capitalLetter ? "fa-check" : "fa-times"
                      }`}
                    ></i>
                  ...
                  <p className={requirements.number ? "valid" : "invalid"}>
                    <i
                      className={`fa ${
                        requirements.number ? "fa-check" : "fa-times"
                      }`}
                    ></i>
                    At least one number
                  </p>
